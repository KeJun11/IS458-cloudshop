  ...
  switch (action.type) {
    ...
    case 'ADD_TO_CART': {
      ...
      if (existingItemIndex >= 0) {
        ...
        updatedItems = [...state.cart.items, { productId: product.id, quantity, product }]
      }
      
      const total = updatedItems.reduce((sum, item) => {
        const itemProduct = item.product || state.products.find(p => p.id === item.productId)
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const actions = {
    loadProducts: async () => {
      ...
      try {
        // For development, use mock data. In production, this will call the API
        const products = mockProducts // await apiService.getProducts()
    ...
    addToCart: async (product: Product, quantity = 1) => {
      try {
        // For development, update local state. In production, this will call the API
        ...
        // const updatedCart = await apiService.addToCart(state.user!.id, product.id, quantity)
    ...
    updateCartItem: async (productId: string, quantity: number) => {
      try {
        ...
        // In production, uncomment this:
        // const updatedCart = await apiService.updateCartItem(state.user!.id, productId, quantity)
        // dispatch({ type: 'SET_CART', payload: updatedCart })
      } catch (error) {
    ...
    removeFromCart: async (productId: string) => {
      try {
        ...
        // In production, uncomment this:
        // const updatedCart = await apiService.removeFromCart(state.user!.id, productId)
        // dispatch({ type: 'SET_CART', payload: updatedCart })
      } catch (error) {
    ...
    loadRecommendations: async () => {
      try {
        if (state.user) {
          // For development, use mock recommendations. In production, this will call the API
          const recommendations = mockProducts.slice(0, 3) // await apiService.getRecommendations(state.user.id)
